...
const WeatherWidget = ({ date, accentColor }: { date: string; accentColor: string }) => {
  ...
  const day = d.toLocaleDateString("vi-VN", { weekday: "long", day: "numeric", month: "long" });
            ...
            <div>
              ...
              <div className="flex items-end gap-3 mt-4">
                ...
                <div>
                  ...
                  <p className="font-body text-sm text-muted-foreground">Nắng nhẹ · trời quang</p>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              {[
                { icon: Thermometer, label: "Cảm giác", value: "30°" },
                { icon: Droplets, label: "Độ ẩm", value: "62%" },
                { icon: Wind, label: "Gió", value: "8 km/h" },
              ].map((s) => (
                <div
                  key={s.label}
